...
All notable changes to LibreLiveTopology will be documented in this file.
...
- **JSON export round-trip improvements**: `toJsonModel()` now includes a `_format: "librelivetopology-map-v1"` version stamp. Import form auto-fills map `name` and `title` from the uploaded JSON file. New `Map::createFromJsonData()` static method extracts the import logic for reuse.
...
- **CLI commands for map management**: Four new `lnms` commands — `librelivetopology:create-map`, `librelivetopology:list-maps`, `librelivetopology:export`, `librelivetopology:discover`. Registered via Artisan through the plugin's ServiceProvider. Reuses existing `MapService::createMap()` and `Map::toJsonModel()` — no backend refactoring needed.
...
### Removed
- **Dead auto-save code**: auto-save was removed from the editor in v1.5.0 (dead code referencing non-existent elements), but the trigger path lingered. Removed: `resources/js/versioning.js` (14KB, orphaned — not loaded by any view; the version-history UI was rebuilt inline in `editor.blade.php` for v1.8.0), the unreachable `MapVersionController::autoSave()` method (no route registered), the `auto_save`/`auto_save_interval` config keys in `config/config.php`, `config/librelivetopology.docker.php`, and `src/LibreLiveTopology.php` defaults, the `auto_save` validation field in `SaveMapVersionRequest`, the `auto_save_enabled`/`auto_save_interval` keys from the version `index()` metadata response, and the unused `$autoSave` parameter from `MapVersionService::createVersion()` (it was never persisted — no `auto_save` column on the model). The `store()` callsite and `tests/integration_version_test.php` callsites updated to the new signature. Tests that asserted `versioning.js` content removed/updated.
- **Dead FormRequest and cache classes**: removed `CreateLinkRequest`, `CreateNodeRequest` (never type-hinted by any controller — `MapLinkController`/`MapNodeController` use inline `$request->validate()`), and `MapCacheService` (zero callers; eager-load fixes it once held were superseded by the v1.7.0 N+1 elimination). `CreateNodeRequest`'s `sanitize` custom validation rule was never registered, so the class would have failed at runtime had it been injected. Operators gain a smaller, safer dependency surface with no behavior change.

### Fixed
- **Node label normalization consistency**: `SaveMapRequest` stripped HTML tags and trimmed node labels on bulk save, but the single-node `create`/`update`/`store` endpoints (`MapNodeController` → `NodeService`) persisted raw validated strings — so a label saved one way could differ from the same label saved the other way. Extracted a shared `NodeLabelNormalizer` (`strip_tags(trim())`, no `htmlspecialchars` — render-time escaping in embed/editor handles that) called from both `NodeService` and `SaveMapRequest`. All write paths now use `normalizeOrThrow()`, which throws `InvalidArgumentException` on empty-after-strip (e.g. `<b></b>`, which passes the `nullable|string` bulk-save rule and the `required|string` single-node rule but holds no text). The single-node controller paths (`create`/`update`/`store`) catch it and return 422. `MapController::save` moves `sanitize()` inside its existing try/catch and maps the exception to 422 (was 400) so bulk save surfaces the same invariant error instead of persisting an empty label. `storeNodes` runs inside a DB transaction so a bad label in any node rolls back the whole bulk replace. Unit-tested in `NodeLabelNormalizerTest`.
...
- **Plugin settings page never loaded** ("Missing view."): `Settings::authorize()` received an empty `User` from the container (not the authenticated session user) via `PluginManager::hooksFor()` → `app()->call()`, so the hook was filtered out and `PluginSettingsController` fell back to the `plugins.missing` view. Now resolves `auth()->user()` first. Additionally, the settings form used flat field names (`poll_interval`, `debug`, …) but `PluginSettingsController::update()` only fills a validated `settings` array — flat names were silently dropped and saves persisted nothing. All inputs now use `settings[...]` array notation. Admins see the 8 LibreLiveTopology settings fields; saves persist to the `plugins.settings` JSON column and round-trip on reload; non-admin users get 403.

### Added
- **Debug-gated per-endpoint traffic logging**: with `LIBRELIVETOPOLOGY_DEBUG=true`, `PortUtilService::linkUtilBits` logs the four raw per-endpoint counters (`a_in`, `a_out`, `b_in`, `b_out`) and the selected `in_bps`/`out_bps` to the application log (via the Laravel `Log` facade — destination follows the host's log config). Server-side only — the live/SSE payload contract is unchanged. This is the diagnostic path for the issue-#11 "95G/77G vs 15G" symptom: the displayed in/out values are `max(A.in, B.out)` / `max(A.out, B.in)`, and an inflated value can only be traced back to one counter via these logs on the affected install.
...
### Added
- **Map tags and filtering**: assign lowercase tags to maps from the editor; tags are stored in `llt_maps.options`. The index view shows tag chips on each card and a tag filter dropdown. Tags are normalized, trimmed, and deduplicated on save.
...
- **Per-map default styles**: define node color/label color and link color/width/via-style defaults per map in the editor's **Default Styles** panel. Styles are stored in `llt_maps.options.default_node_style` and `options.default_link_style`; they merge at render time and do not modify saved node/link objects, so version diffs stay clean. Validation allowlists keys and hex colors.
- **Operational diagnostics page** for administrators: `/plugin/LibreLiveTopology/diagnostics` shows overall health, map/node/link counts, per-check status (database, filesystem, dependencies, configuration, performance), route registration, and writable-path checks. Linked from **Network Maps → Diagnostics** for admin users.
...
### Changed
- **Shared client helpers consolidated**: `resources/js/llt-common.js` now hosts `getCsrfToken()`, `fetchJson()`, `ensureUiHelpers()`, `detectTheme()`, and `observeTheme()` — replacing inline polyfills in `editor.blade.php` and `index.blade.php`. CSRF token lookups use the helper with empty-string fallback instead of `.content` (which throws if the meta tag is absent).
- **Dead duplicate assets removed**: Deleted root-level `js/librelivetopology.js`, `css/librelivetopology.css`, and `resources/js/librelivetopology.js` — stale duplicates of code that lives in the blade views and `resources/css/librelivetopology.css`.
- **Legacy `index.php` fallback fixes**: Map list now selects `title`/`options` from the correct schema (width/height live in the options JSON), version is read from the `VERSION` file, and the CSS asset path points at `resources/css/librelivetopology.css`.
...
- **Toast/loading class name shadowing broke save and all toast feedback** (issue #11): `ui-helpers.js` declared `class LLTToast` and `class LLTLoading`, then assigned `window.LLTToast = new LLTToast()`. In subsequent `<script>` blocks, bare `LLTToast.success(...)` resolved to the class constructor (no `.success` method), not the instance — throwing `TypeError: LLTToast.success is not a function` and crashing the save flow. Renamed to `LLTToastManager`/`LLTLoadingManager` so bare references resolve to the `window` instance.
...
### Fixed
- **Editor save crash when ui-helpers.js fails to load** (issue #11, reopened): `saveMap()` called `LLTLoading.show()` and `LLTToast.*()` which are defined in `resources/js/ui-helpers.js`. If the `asset()` path 404s or serves a stale version missing methods, the save button throws `LLTLoading.show is not a function` and the map never saves. Added a polyfill in `editor.blade.php` and `index.blade.php` that checks whether `LLTLoading`/`LLTToast` exist and have the expected methods; if not, installs safe no-op (loading) and console.log (toast) fallbacks. All direct `LLTToast.*`/`LLTLoading.*` calls in the editor and index views are now safe regardless of whether the external JS file loads.
...
- **Broken frontend routes**: Import form URLs in index.blade.php pointed to `/api/maps/import` instead of `/api/import`. Install route name was `librelivetopology.install.post` instead of `librelivetopology.install.run`. Settings form posted to non-existent `/api/settings`, `/api/backup/create`, `/api/settings/reset` — now uses LibreNMS's `plugin.update` route (via hooks/settings.blade.php) and disabled backup/reset show informational messages.
...
- **Dead view files**: `views/editor-modern.php` (1608 lines), `views/editor.php` (281), `views/view.php` (231) targeted non-existent `/plugin/v1/LibreLiveTopology/` endpoints — deleted.
...
### Changed
- **Installer idempotency**: Quick install now normalizes LibreNMS plugin registration after enablement, keeping one active `LibreLiveTopology` row and removing stale duplicate rows.
- **Install guidance**: README, install, and deployment docs now explain expected `llt_*` validation warnings and JSON-column `utf8mb4_bin` collation warnings.
...
### Fixed
- **Duplicate plugin registration**: Fixed clean reinstall cases where LibreNMS could keep an inactive duplicate `LibreLiveTopology` row in the `plugins` table (issue #9).
...
### Changed
- **LibreNMS package registration**: Quick install now registers LibreLiveTopology as a Composer path package in the LibreNMS root before running package discovery.
- **Install verification**: Quick install now verifies LibreLiveTopology routes and creates required output directories.
...
### Fixed
- **Route discovery failure**: Fixed installs that appeared successful but did not load LibreLiveTopology Laravel routes until Composer package registration was performed manually.
...
- **Demo Mode**: Simulated traffic data for testing without real LibreNMS devices
  - Enable with `LIBRELIVETOPOLOGY_DEMO_MODE=true` environment variable
...
- **Versioning Table Missing**: Added `llt_map_versions` table to `database/setup.php` for fresh installs and upgrades
...
- **Cache Key Collision**: Port metadata and traffic data now use separate cache keys
  - DevicePortLookup: `librelivetopology.port.meta.{id}`
  - PortUtilService: `librelivetopology.port.traffic.{id}`
- **Cache Invalidation**: Removed non-functional wildcard patterns from clearCaches()
- **Version Sync**: Unified version to 1.6.0 across composer.json and LibreLiveTopology.php
...
- **Updated Files**:
  - `composer.json` - Added LibreLiveTopologyServiceProvider
...
- **Loading States**: LLTLoading.show() for async operations
- **Toast Notifications**: LLTToast.success/error/info for feedback
...
- **Module Pattern**: Global window.LibreLiveTopologyVersioning object
...
- **Service Provider**: LibreLiveTopologyServiceProvider registered
...
- **Cache Keys**:
  - librelivetopology:maps:all
  - librelivetopology:map:{id}
  - librelivetopology:map:nodes:{id}
  - librelivetopology:map:links:{id}
  - librelivetopology:map:{id}:editor
  - librelivetopology:devices:map
...
- **JS Helpers**: New ui-helpers.js with LLTToast, LLTLoading, LLTA11y classes
...
- API endpoints: map save (`POST /plugin/LibreLiveTopology/api/maps/{id}/save`), node/link CRUD, autodiscover (`POST /plugin/LibreLiveTopology/map/{id}/autodiscover`).
...
- Routes consolidated under `plugin/LibreLiveTopology/...` paths.
...
- Added a compatibility migration to backfill missing columns (e.g., `llt_maps.title`) on older installs. Run migrations on upgrade.
...
### Added
- Initial file-based librelivetopology implementation
