# LibreLiveTopology Performance Notes
...
LibreLiveTopology renders maps in browser canvas views and reads live traffic from LibreNMS data sources. The main cost centers are:
...
The live/embed/sse paths preload related data before serializing map data:
...
Measure query counts and latency in the target deployment. This project does not publish a verified query-count or throughput benchmark.

## Caching

There is no dedicated cache service. Caching is inline `Cache::remember` in the
services that fetch data, each with a `librelivetopology.*` key scoped by resource id
so writes invalidate the relevant data without wildcard cache clearing:

- `PortUtilService` — per-port traffic (`librelivetopology.port.traffic.{id}`) and per-device aggregate (`librelivetopology.device.{id}.aggregate`).
...
Cache TTL follows `librelivetopology.cache_ttl` (default 300s). `RrdDataService` caches are request-local only — they reset per request to avoid stale RRD reads across polls.
...
LibreLiveTopology stores maps, nodes, links, templates, and versions in `llt_*` tables inside the LibreNMS database.

Useful checks:

```sql
SHOW TABLES LIKE 'llt_%';
SELECT COUNT(*) FROM llt_maps;
SELECT COUNT(*) FROM llt_nodes;
SELECT COUNT(*) FROM llt_links;
```
