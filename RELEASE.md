...
Pre-release validation flow and manual QA for tagging a LibreLiveTopology release.
...
1. **Syntax check all changed PHP files**
   ...
   ```bash
   docker run --rm --entrypoint php -v "$PWD":/opt/librenms/html/plugins/LibreLiveTopology \
     -w /opt/librenms/html/plugins/LibreLiveTopology librenms/librenms:latest \
     -l <file>
   ```
...
- **Install**: on a clean LibreNMS install, run `quick-install.sh`; confirm
  LibreLiveTopology routes appear under the LibreNMS plugin menu.
...
LibreLiveTopology follows [Semantic Versioning](https://semver.org/) (`MAJOR.MINOR.PATCH`).
