...
LibreLiveTopology has two different versioning concerns:
...
LibreLiveTopology follows [Semantic Versioning](https://semver.org/spec/v2.0.0.html) for releases:
...
v1.7.0 standardized authorization on a simple admin-only model: maps have no per-user ownership and there is no `user_id` column on `llt_maps`. Any authenticated LibreNMS user can read maps, templates, lookups, and health detail; all `POST`/`PUT`/`PATCH`/`DELETE` operations require an admin (`hasGlobalAdmin()`, `isAdmin()`, or `level >= 10`), enforced by the shared `AdminCheck` trait in each controller. Public health/readiness/liveness probes remain unauthenticated. Earlier per-map policy scaffolding (`MapPolicy`, `NodePolicy`) has been removed. See [API.md](API.md) for the per-route breakdown.
...
Map versioning stores map snapshots in the `llt_map_versions` table. It is intended for rollback and auditability while editing maps.
...
- Keep database backups that include `llt_map_versions`.
