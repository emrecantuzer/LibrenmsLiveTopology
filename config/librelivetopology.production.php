<?php
/**
 * LibreLiveTopology Production Configuration Template
 * Copy this to librelivetopology.php and adjust for your environment
 */

return [
    // Basic settings
    'default_width' => 800,
    'default_height' => 600,
    'poll_interval' => 300, // 5 minutes
    
    // Data sources
    'rrd_base' => env('LIBRENMS_RRD_BASE', '/opt/librenms/rrd'),
    'enable_local_rrd' => true,
    'enable_api_fallback' => true,
    'api_token' => env('LIBRENMS_API_TOKEN', ''),
    
    // Performance
    'cache_ttl' => 300,
    'thresholds' => [50, 80, 95],
    
    // Logging (production = minimal logging)
    'debug' => false,
    'log_level' => 'error',
    'log_file' => '/var/log/librenms/librelivetopology.log',
    
    // Colors
    'colors' => [
        'node_up' => '#22a06b',
        'node_down' => '#d64545',
        'link_normal' => '#3b82a0',
        'link_warning' => '#d99a21',
        'link_critical' => '#d64545',
    ],
    
    // Display
    'link_style' => 'curved',
    'show_bandwidth' => true,
    'show_node_metrics' => true,
    
    // Security
    'allow_embed' => true,
    'max_image_size' => 2048, // KB
];
