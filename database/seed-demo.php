<?php
/**
 * LibreLiveTopology Demo Data Seeder
 *
...
echo "LibreLiveTopology Demo Data Seeder\n";
echo "=============================\n\n";
...
try {
    ...
    if (class_exists('Illuminate\Support\Facades\DB')) {
        ...
        $existing = $db::table('llt_maps')->where('name', 'demo-network')->first();
        if ($existing) {
            echo "Demo map already exists (ID: {$existing->id})\n";
            echo "Delete it first if you want to recreate: DELETE FROM llt_maps WHERE name='demo-network'\n";
            exit(0);
        }

        // Create demo map
        $mapId = $db::table('llt_maps')->insertGetId([
            'name' => 'demo-network',
            'title' => 'Demo Network Topology',
            'description' => 'Sample network for testing LibreLiveTopology',
            'width' => 1200,
            'height' => 700,
            'options' => json_encode(['background' => '#ffffff', 'width' => 1200, 'height' => 700]),
            'created_at' => now(),
            'updated_at' => now(),
        ]);
        ...
        foreach ($nodes as $node) {
            $nodeIds[] = $db::table('llt_nodes')->insertGetId([
                'map_id' => $mapId,
                'label' => $node['label'],
                'x' => $node['x'],
                'y' => $node['y'],
        ...
        echo "\nView your demo map at: /plugin/LibreLiveTopology/embed/$mapId\n";
        echo "\nTip: Enable demo mode for simulated traffic:\n";
        echo "  export LIBRELIVETOPOLOGY_DEMO_MODE=true\n";
        exit(0);
    }
} catch (Exception $e) {
