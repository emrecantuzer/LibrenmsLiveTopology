# LibreLiveTopology Embed Viewer

## Overview

- Route: `plugin/LibreLiveTopology/embed/{map}`
...
- Polls `/plugin/LibreLiveTopology/api/maps/{id}/live` on interval
- If `sse=1`, subscribes to `/plugin/LibreLiveTopology/api/maps/{id}/sse`
...
|------------|---------|-------------|
| `librelivetopology.link_style` | `straight` | Default via_style for links: `straight`, `angled`, `curved` |
| `librelivetopology.show_bandwidth` | `true` | Show bandwidth labels (Gb/s, Mb/s) |
| `librelivetopology.show_percentages` | `true` | Show utilization percentage labels |
| `librelivetopology.show_node_metrics` | `true` | Show CPU/memory utilization overlay on nodes |
| `librelivetopology.thresholds` | `[50,80,95]` | Utilization thresholds for green/yellow/red coloring |
...
- NOC wall rotating maps every 30 seconds:  
  `plugin/LibreLiveTopology/embed/1?kiosk=1&cycle=30`
- Static kiosk where clicks stay in the same tab:  
  `plugin/LibreLiveTopology/embed/1?kiosk=1&target=self`
- Static wall, no cycling:  
  `plugin/LibreLiveTopology/embed/1?kiosk=1`
