<?php
/**
 * LibreLiveTopology legacy entry fallback
 *
 * Canonical UI is registered via LibreLiveTopologyProvider routes
 * (GET /plugin/LibreLiveTopology → PageController). This file remains for
...
function librelivetopology_db_ready() {
    try {
        return \Illuminate\Support\Facades\Schema::hasTable('llt_maps');
    } catch (\Exception $e) {
        return false;
    }
}
...
// Get all maps
function librelivetopology_get_maps() {
    if (!librelivetopology_db_ready()) {
        return collect();
    }

    try {
        // Schema: name/title/options/timestamps — width/height live in options JSON.
        return \Illuminate\Support\Facades\DB::table('llt_maps')
...
function librelivetopology_plugin_version() {
...
// Main plugin data
// Note: canonical UI is the Laravel route /plugin/LibreLiveTopology (PageController).
// This file is a legacy fallback entry point for tools that include index.php directly.
$title = 'LibreLiveTopology - Network Weather Maps';
$maps = librelivetopology_get_maps();
$plugin_version = librelivetopology_plugin_version();
...
<head>
    ...
    <!-- Include LibreLiveTopology CSS -->
    <link href="/plugins/LibreLiveTopology/resources/css/librelivetopology.css" rel="stylesheet">

    <style>
        .librelivetopology-hero {
...
<body>
    <div class="container-fluid">
        <!-- Hero Section -->
        <div class="librelivetopology-hero">
            <div class="text-center">
                ...
                <div class="action-buttons justify-content-center">
                    ...
                    <a href="/plugin/settings/LibreLiveTopology" class="btn btn-outline-light btn-lg">
                ...
                <?php foreach ($maps as $map): ?>
                    <div class="map-card">
                        ...
                        <div class="map-card-footer">
                            <a href="/plugin/LibreLiveTopology/view/<?php echo $map->id; ?>" class="btn btn-primary btn-sm flex-fill">
                                <i class="fas fa-eye"></i> View
                            </a>
                            <a href="/plugin/LibreLiveTopology/editor/<?php echo $map->id; ?>" class="btn btn-warning btn-sm flex-fill">
        ...
        <?php if (!librelivetopology_db_ready()): ?>
            <div class="alert alert-warning mt-4">
                <h4><i class="fas fa-exclamation-triangle"></i> Database Setup Required</h4>
                <p>The LibreLiveTopology database tables need to be created.</p>
                <pre>cd /opt/librenms/html/plugins/LibreLiveTopology && php database/setup.php</pre>
            </div>
        <?php endif; ?>
    </div>
    ...
    function updateStats() {
        fetch('/plugin/v1/LibreLiveTopology?ajax_action=get-stats')
    ...
    function submitCreateMap() {
        ...
        fetch('/plugin/v1/LibreLiveTopology', {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                ...
                // Redirect to editor
                window.location.href = '/plugin/v1/LibreLiveTopology/editor/' + data.id;
            } else {
