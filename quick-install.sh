#!/bin/bash
# LibreLiveTopology Quick Install
# Works with both native and Docker LibreNMS installations

set -e

echo "LibreLiveTopology Quick Install"
echo "=========================="
...
if [ ! -f "$PLUGIN_DIR/composer.json" ]; then
    echo "Error: Please run this from the LibreLiveTopology plugin directory"
    exit 1
fi
...
if [ "$(id -u)" -eq 0 ] && [ "$IN_DOCKER" = false ] && [ "${LIBRELIVETOPOLOGY_ALLOW_ROOT:-0}" != "1" ]; then
    ...
    echo ""
    echo "If you really need to override this guard, set LIBRELIVETOPOLOGY_ALLOW_ROOT=1."
    exit 1
fi
...
if [ -f "$LIBRENMS_PATH/composer.json" ]; then
    cd "$LIBRENMS_PATH"
    if ! COMPOSER_ALLOW_SUPERUSER=1 composer config repositories.librelivetopology "{\"type\":\"path\",\"url\":\"$PLUGIN_DIR\",\"options\":{\"symlink\":true}}" 2>&1; then
        echo "Error: Could not register LibreLiveTopology as a Composer path repository"
        exit 1
    fi

    if ! llt_run_as_librenms "FORCE=1 COMPOSER_ALLOW_SUPERUSER=1 composer require 'librenms/librelivetopology:*' --with-dependencies --no-interaction 2>&1"; then
        echo "Error: Could not add LibreLiveTopology to the LibreNMS Composer install"
        echo "  Try running from $LIBRENMS_PATH:"
        echo "  composer config repositories.librelivetopology '{\"type\":\"path\",\"url\":\"$PLUGIN_DIR\",\"options\":{\"symlink\":true}}'"
        echo "  FORCE=1 composer require 'librenms/librelivetopology:*' --with-dependencies --no-interaction"
        exit 1
    fi

    if [ -f "$LIBRENMS_PATH/artisan" ]; then
        llt_run_as_librenms "php artisan package:discover 2>/dev/null" || echo "  Warning: package:discover failed"
    fi
else
...
if [ -f "$LIBRENMS_PATH/lnms" ]; then
    cd "$LIBRENMS_PATH"
    llt_run_as_librenms "./lnms plugin:enable LibreLiveTopology 2>/dev/null" || echo "  Note: Plugin may already be enabled"

    echo "  Normalizing LibreLiveTopology plugin registration..."
    php <<'PHP'
<?php
...
try {
    ...
    if ($rows->isEmpty()) {
        echo "  Warning: No LibreLiveTopology plugin row found after enable step\n";
        exit(0);
    }
...
PHP
    ...
    echo "  Skipped: lnms not found (enable manually with: ./lnms plugin:enable LibreLiveTopology)"
fi
...
if [ -f "$LIBRENMS_PATH/artisan" ]; then
    cd "$LIBRENMS_PATH"
    llt_run_as_librenms "php artisan optimize:clear 2>/dev/null" || echo "  Warning: optimize:clear failed (may need sudo)"
    llt_run_as_librenms "php artisan route:clear 2>/dev/null" || echo "  Warning: route:clear failed"
    llt_run_as_librenms "php artisan view:clear 2>/dev/null" || echo "  Warning: view:clear failed"
    llt_run_as_librenms "php artisan config:clear 2>/dev/null" || echo "  Warning: config:clear failed"
    llt_run_as_librenms "php artisan cache:clear 2>/dev/null" || echo "  Warning: cache:clear failed"

    if llt_run_as_librenms "php artisan route:list 2>/dev/null" | grep -qiE "librelivetopology|llt"; then
        echo "  LibreLiveTopology routes detected"
    else
        echo "  Warning: LibreLiveTopology routes were not detected"
        echo "  Check with: cd $LIBRENMS_PATH && php artisan route:list | grep -iE 'librelivetopology|llt'"
    fi
else
...
echo ""
echo "Next steps:"
echo "  1. Visit: https://your-server/plugin/LibreLiveTopology"
echo "  2. Create your first map!"
echo "  3. If LibreNMS validate.php flags llt_* as extra tables, keep them."
echo "     JSON column collation warnings on llt_* JSON columns are expected."
echo ""
echo "Optional - Create demo data:"
echo "  php $PLUGIN_DIR/database/seed-demo.php"
echo ""
echo "Troubleshooting: see INSTALL.md in the plugin directory"
