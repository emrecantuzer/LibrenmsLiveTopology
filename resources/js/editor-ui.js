/**
 * LibreLiveTopology editor — data layer and UI chrome.
 *
 ...
 * `window.LLT.EditorState` (see editor-state.js). Rendering / node / link
...
function validateAndApplyCanvasResize(newWidth, newHeight) {
    ...
    const applyCanvasResize = () => {
        ...
        renderEditor();
        LLTToast.info(`Canvas resized to ${newWidth}x${newHeight}`, { duration: 2000 });
    };
...
function handleKeyDown(event) {
    ...
    // F: Fit the topology without changing saved positions.
...
function saveState() {
    const state = {
        mapWidth: S.mapWidth, mapHeight: S.mapHeight,
...
function undo() {
    ...
    if (S.undoStack.length === 0) {
        LLTToast.info('Nothing to undo', { duration: 1500 });
        return;
    }
    ...
    restoreHistoryDimensions(previousState);
    S.selectedNodes = [];
    S.selectedNode = null;
    populateNodeProperties(null);
    renderEditor();
    ...
    updateStatusCounts();
    markUnsaved();
    LLTToast.info('Undone', { duration: 1000 });
}

function redo() {
    if (S.replay?.active || S.replay?.loading) return;
    if (S.redoStack.length === 0) {
        LLTToast.info('Nothing to redo', { duration: 1500 });
        return;
    }
    ...
    restoreHistoryDimensions(redoState);
    S.selectedNodes = [];
    S.selectedNode = null;
    populateNodeProperties(null);
    renderEditor();
    ...
    updateStatusCounts();
    markUnsaved();
    LLTToast.info('Redone', { duration: 1000 });
}

function restoreHistoryDimensions(state) {
    S.mapWidth = state.mapWidth || S.mapWidth;
    S.mapHeight = state.mapHeight || S.mapHeight;
...
function loadMapData(id) {
    return fetch(S.uris.maps + '/' + id + '/json', {
        ...
        .then(data => {
            ...
            S.nodes = (data.nodes || []).map(node => ({
                ...
                topologyRole: node.meta?.topology_role || 'auto',
                meta: node.meta || {},
            }));
        ...
        .catch(error => {
            ...
            LLTToast.error('Failed to load map data: ' + error.message + '. Saving is disabled until the map loads.', { duration: 5000 });
        });
}
...
function initDeviceAutocomplete(searchId, resultsId, onSelect) {
    ...
    if (searchId === 'device-search') {
        search.addEventListener('focus', function () {
            if (this.value.trim() || S.devicesCache.length) return;
            dropdown.innerHTML = '<div class="ac-loading">Loading devices...</div>';
            dropdown.classList.add('show');
            fetch(S.uris.devices)
                .then(r => {
                    if (!r.ok) throw new Error('HTTP ' + r.status);
                    return r.json();
                })
                .then(data => setItems(Array.isArray(data) ? data : (data.devices || [])))
                .catch(function () {
                    dropdown.innerHTML = '<div class="ac-empty">Unable to load devices</div>';
                });
        });
    }
...
function normalizedLinkStyleForSave(link) {
    const style = { ...(link.style || {}) };
    if (style.via_style !== 'curved') return style;

    const validPoints = Array.isArray(style.via_points)
        && style.via_points.some(point => Number.isFinite(Number(point?.x)) && Number.isFinite(Number(point?.y)));
    if (validPoints) return style;

    const source = S.nodes.find(node => String(node.id) === String(link.srcId));
    const target = S.nodes.find(node => String(node.id) === String(link.dstId));
    if (!source || !target) return style;

    const dx = target.x - source.x;
    const dy = target.y - source.y;
    const length = Math.hypot(dx, dy) || 1;
    style.via_points = [{
        x: Math.max(0, Math.min(S.mapWidth, (source.x + target.x) / 2 - (dy / length) * 36)),
        y: Math.max(0, Math.min(S.mapHeight, (source.y + target.y) / 2 + (dx / length) * 36)),
    }];
    link.style = style;
    return style;
}

function applySavedMapData(data) {
...
function editorSavePayload(mapName, mapTitle, mapWidth, mapHeight) {
    ...
    return {
        ...
        nodes: S.nodes.map(n => ({
            ...
            meta: { ...n.meta, interface_id: n.interfaceId || null, device_type: n.deviceType || 'auto', topology_role: n.topologyRole || 'auto' },
        })),
        links: S.links.map(l => ({
            src_node_id: l.srcId,
            dst_node_id: l.dstId,
            ...
            style: normalizedLinkStyleForSave(l),
        })),
    };
}

function saveMap() {
    if (S.replay?.active || S.replay?.loading) {
        LLTToast.warning('Return to Live before saving your map.', { duration: 3000 });
        return;
    }
    if (S.discoveryInProgress || S.saveInProgress) return;
    ...
    if (!mapName) {
        LLTToast.error('Please enter a map name.', { duration: 3000 });
        return;
    }
    ...
    if (S.mapId && !S.mapDataLoaded) {
        if (S.mapDataLoadFailed) {
            LLTToast.error('Cannot save: map data failed to load. Reload the page and try again.', { duration: 5000 });
        } else {
            LLTToast.warning('Map is still loading, please wait a moment and try again.', { duration: 3000 });
        }
        return;
    }
    const baseHeaders = {
    ...
    if (!S.mapId) {
        ...
        const initialTopology = editorSavePayload(mapName, mapTitle, mapWidth, mapHeight);
        return fetch(S.uris.map, {
            method: 'POST',
            headers: baseHeaders,
            ...
            .then(async data => {
                if (data.success) {
                    if (!data.map?.id) throw new Error('Created map response is missing its ID. Your draft is still in this editor.');
                    // Keep the ID immediately, so a failed content save can be
                    // retried without creating a duplicate map or losing draft.
                    S.mapId = data.map.id;
                    S.mapDataLoaded = true;
                    const response = await fetch(S.uris.maps + '/' + S.mapId + '/save', {
                        method: 'POST', headers: baseHeaders, body: JSON.stringify(initialTopology),
                    });
                    const saved = await response.json();
                    if (!response.ok || !saved.success) throw new Error(saved.message || 'Could not save initial topology. Retry Save.');
                    markSaved();
                    window.location.href = data.redirect || S.uris.editor + '/' + S.mapId;
                } else {
                    LLTToast.error('Failed to create map: ' + (data.message || 'Unknown error'), { duration: 3000 });
                }
            })
            .catch(error => {
                LLTLoading.hide();
                LLTToast.error('Error creating map: ' + error.message, { duration: 3000 });
            })
            .finally(() => { S.saveInProgress = false; LLTLoading.hide(); });
    }

    LLTLoading.show('Saving map...');
    const payload = editorSavePayload(mapName, mapTitle, mapWidth, mapHeight);

    return fetch(S.uris.maps + '/' + S.mapId + '/save', {
        method: 'POST',
        headers: baseHeaders,
        body: JSON.stringify(payload),
    })
        .then(async r => {
            // Surface non-2xx (403 admin gate, 419 CSRF, 500 server) before
            // trying to parse a success body — but for 422 validation
            // errors, read the real per-field message first (e.g. an
            // out-of-range via_point) instead of a bare "HTTP 422" that
            // hides which field failed and makes the save look like it
        ...
        .then(data => {
            LLTLoading.hide();
            if (data.success) {
                if (!applySavedMapData(data.map)) {
                    throw new Error('Server saved the map but did not return canonical map data. Reload before editing again.');
                }
                LLTToast.success('Map saved successfully!', { duration: 3000 });
                markSaved();
            } else {
                LLTToast.error('Error saving map: ' + (data.message || 'Unknown error'), { duration: 3000 });
            }
        })
        .catch(err => {
            LLTLoading.hide();
            LLTToast.error('Error saving map: ' + err.message, { duration: 3000 });
        })
        .finally(() => { S.saveInProgress = false; });
}
...
function autoDiscoverMap() {
    if (S.discoveryInProgress || S.saveInProgress) return;
    if (S.replay?.active || S.replay?.loading) {
        LLTToast.warning('Return to Live before running auto-discovery.', { duration: 3000 });
        return;
    }
    if (!S.mapId) {
        LLTToast.warning('Save the map first, then run auto-discovery.', { duration: 4000 });
        return;
    }
    if (!S.mapDataLoaded) {
        if (S.mapDataLoadFailed) {
            LLTToast.error('Cannot run auto-discovery: map data failed to load. Reload the page and try again.', { duration: 5000 });
        } else {
            LLTToast.warning('Map is still loading, please wait a moment and try again.', { duration: 3000 });
        }
        return;
    ...
    if (S.hasUnsavedChanges) {
        LLTToast.warning('Save your changes before discovering devices. Discovery preserves the saved layout.', { duration: 5000 });
        return;
    }
    const wasEmpty = S.nodes.length === 0;
    S.discoveryInProgress = true;
    const discoveryButton = document.getElementById('autodiscover-btn');
    if (discoveryButton) discoveryButton.disabled = true;
    LLTLoading.show('Running auto-discovery...');
    return fetch(S.uris.map + '/' + S.mapId + '/autodiscover', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json',
        ...
        .then(async data => {
            LLTLoading.hide();
            if (data.success) {
                LLTToast.success(data.message, { duration: 5000 });
                // Reload so the discovered nodes/links render on the canvas.
                S.mapDataLoaded = false;
                S.mapDataLoadFailed = false;
                const loaded = await loadMapData(S.mapId);
                if (loaded && wasEmpty && S.nodes.length) {
                    saveState();
                    if (runLayeredNetworkLayout()) {
                        renderEditor();
                        markUnsaved();
                        LLTToast.info('Discovered topology arranged by network neighbours. Save to publish this layout.', { duration: 6000 });
                    }
                }
            } else {
                LLTToast.error(data.message || 'Auto-discovery failed.', { duration: 5000 });
            }
        })
        .catch(err => {
            LLTLoading.hide();
            LLTToast.error('Error running auto-discovery: ' + err.message, { duration: 5000 });
        })
        .finally(() => {
            S.discoveryInProgress = false;
            if (discoveryButton) discoveryButton.disabled = false;
        });
}
...
function exportJson() {
    ...
    LLTToast.success('Map exported as JSON', { duration: 2000 });
}
...
function bulkDeleteSelected() {
    ...
    if (toDelete.length === 0) {
        LLTToast.info('Nothing selected to delete', { duration: 2000 });
        return;
    }
                ...
                fetch(S.uris.map + '/' + S.mapId + '/node/' + n.dbId, {
                    ...
                    .finally(() => {
                        ...
                        if (remaining === 0) {
                            confirmCleanup();
                            if (failed) {
                                LLTToast.warning('Some nodes could not be deleted. Reload the map to refresh.', { duration: 5000 });
                            }
                        }
                    });
            });
