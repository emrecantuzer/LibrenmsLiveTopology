/**
 * LibreLiveTopology editor — version history UI.
 *
 ...
 * Loaded last: it depends on `window.LLT.EditorState` (editor-state.js) and
...
(function () {
    ...
    function csrfHeaders() {
        return { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': LLT.getCsrfToken() };
    }

    function toast(method, msg, opts) {
        try {
            if (window.LLTToast && typeof window.LLTToast[method] === 'function') {
                window.LLTToast[method](msg, opts);
            }
        } catch (e) { /* toast failure must never break the UI */ }
    }

    function fetchVersions() {
        return LLT.fetchJson(`${API_MAPS}/${S.mapId}/versions?_=${Date.now()}`, {
            cache: 'no-store',
    ...
    function loadDiff(v1Id, v2Id) {
        LLT.fetchJson(`${API_VERSIONS}/${v1Id}/compare/${v2Id}`)
            .then(data => {
    ...
    function saveVersion() {
        ...
        LLT.fetchJson(`${API_MAPS}/${S.mapId}/versions`, {
            method: 'POST',
            headers: csrfHeaders(),
    ...
    function restoreVersion(versionId) {
        ...
        showEditorConfirm(
            ...
            'btn-primary',
            () => {
                LLT.fetchJson(`${API_VERSIONS}/${versionId}/restore`, {
                    method: 'POST',
                    headers: csrfHeaders()
                })
                    .then(() => {
    ...
    function deleteVersion(versionId) {
        ...
        showEditorConfirm(
            ...
            'btn-danger',
            () => {
                LLT.fetchJson(`${API_VERSIONS}/${versionId}`, {
                    method: 'DELETE',
                    headers: csrfHeaders()
                })
                    .then(() => {
