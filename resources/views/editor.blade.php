...
@push('styles')
<link rel="stylesheet" href="{{ asset('plugins/LibreLiveTopology/resources/css/librelivetopology.css') }}">
<link rel="stylesheet" href="{{ asset('plugins/LibreLiveTopology/resources/css/loading.css') }}">
<link rel="stylesheet" href="{{ asset('plugins/LibreLiveTopology/resources/css/toast.css') }}">
<link rel="stylesheet" href="{{ asset('plugins/LibreLiveTopology/resources/css/a11y.css') }}">
<style>
/* ===== Light Mode (Default) ===== */
.editor-container {
...
.advanced-panel {
    ...
    padding: 6px 12px; background: var(--editor-panel-header-bg);
    border-bottom: 1px solid var(--editor-topbar-border); color: var(--editor-text);
}
.advanced-panel[hidden] { display: none; }
.advanced-panel input[type="range"] { flex: 1 1 240px; min-width: 140px; }
.advanced-panel small { min-width: 120px; }
...
<div class="editor-container">
    ...
    <div class="editor-toolbox">
        ...
        <button type="button" class="tool-btn" onclick="autoLayoutNodes()" title="Auto-Layout Nodes (declutter overlapping links)" aria-label="Automatically arrange nodes to reduce overlap">
        ...
        <button type="button" class="tool-btn" onclick="fitEditorTopology()" title="Fit topology (F)" aria-label="Fit all devices and routes without moving them">
    ...
    <div class="editor-canvas-area">
        ...
        <div class="editor-topbar">
            <div class="d-flex align-items-center">
                <a href="{{ url('plugin/LibreLiveTopology') }}" class="btn btn-sm btn-outline-secondary mr-2">
            ...
            <div class="d-flex align-items-center">
                ...
                @if($map)
                <a href="{{ url('plugin/LibreLiveTopology/embed/' . $map->id) }}" target="_blank" rel="noopener noreferrer" class="btn btn-sm btn-outline-info mr-2" title="Preview" aria-label="Preview map">
                ...
                <button type="button" class="btn btn-sm btn-outline-warning ml-1" id="replay-toggle" aria-label="Toggle historical replay">
                    <i class="fas fa-clock"></i> Replay
                </button>
                <button type="button" class="btn btn-sm btn-outline-danger ml-1" id="simulation-toggle" aria-label="Toggle what-if simulation">
                    <i class="fas fa-flask"></i> Simulate
                </button>
                <button type="button" class="btn btn-sm btn-outline-secondary ml-1" id="audio-toggle" aria-label="Toggle network audio">
        ...
        <div id="advanced-panel" class="advanced-panel" hidden>
            <span id="advanced-mode-label">Live mode</span>
            <input id="replay-slider" type="range" min="{{ now()->subDays(30)->timestamp }}" max="{{ now()->timestamp }}" value="{{ now()->timestamp }}" step="60" aria-label="Historical replay time">
            <button type="button" class="btn btn-sm btn-outline-secondary" id="live-mode-btn">Live</button>
            <button type="button" class="btn btn-sm btn-outline-danger" id="clear-simulation-btn">Clear simulation</button>
            <small id="advanced-status" class="text-muted"></small>
        </div>
        @endif
        ...
        <div class="editor-canvas-wrap">
            <canvas id="map-canvas"
                    width="{{ $map->width ?? config('librelivetopology.default_width', 800) }}"
                    height="{{ $map->height ?? config('librelivetopology.default_height', 600) }}">
            </canvas>
            <!-- Minimap -->
    ...
    <div class="editor-sidebar">
        ...
        <div class="panel" id="node-properties-card" style="display: none;">
            ...
            <div class="panel-body">
                ...
                <div class="form-group mb-2">
                    <label class="form-label" for="node-prop-device-type">Device Visual</label>
                    <select class="form-control form-control-sm" id="node-prop-device-type">
                        <option value="auto">Auto detect</option>
                        <option value="paloalto">Palo Alto</option>
                        <option value="netscaler">NetScaler</option>
                        <option value="fortinet">Fortinet / FortiGate</option>
                        <option value="firewall">Generic firewall</option>
                        <option value="router">Router</option>
                        <option value="switch">Switch</option>
                        <option value="aruba">Aruba Switch</option>
                        <option value="brocade">Brocade Switch</option>
                        <option value="server">Server</option>
                    </select>
                </div>
                <div class="form-group mb-2">
                    <label class="form-label" for="node-prop-topology-role">Network role</label>
                    <select class="form-control form-control-sm" id="node-prop-topology-role">
                        <option value="auto">Infer from name and neighbours</option>
                        <option value="core">Core / Spine</option>
                        <option value="distribution">Distribution / Aggregation</option>
                        <option value="access">Access / Leaf / Edge</option>
                    </select>
                    <small class="text-muted">Used by Auto-Layout; changing a role keeps the current position.</small>
                </div>
                ...
                <div class="form-group mb-2">
                    ...
                    <div class="d-flex align-items-center">
                        <input type="number" class="form-control form-control-sm" id="map-width"
                               value="{{ $map->width ?? config('librelivetopology.default_width', 800) }}" min="100" max="4096">
                        <span class="mx-2 text-muted">×</span>
                        <input type="number" class="form-control form-control-sm" id="map-height"
                               value="{{ $map->height ?? config('librelivetopology.default_height', 600) }}" min="100" max="4096">
                    </div>
                </div>
                <div class="form-group mb-2">
...
<div class="modal fade" id="linkModal" tabindex="-1">
    <div class="modal-dialog">
        ...
        @section('scripts')
        @php
            $editorAssetVersion = max(array_map('filemtime', [
                public_path('plugins/LibreLiveTopology/resources/js/llt-common.js'),
                public_path('plugins/LibreLiveTopology/resources/js/ui-helpers.js'),
                public_path('plugins/LibreLiveTopology/resources/js/editor-state.js'),
                public_path('plugins/LibreLiveTopology/resources/js/topology-ports.js'),
                public_path('plugins/LibreLiveTopology/resources/js/topology-layout.js'),
                public_path('plugins/LibreLiveTopology/resources/js/editor-canvas.js'),
                public_path('plugins/LibreLiveTopology/resources/js/editor-nodes.js'),
                public_path('plugins/LibreLiveTopology/resources/js/editor-links.js'),
                public_path('plugins/LibreLiveTopology/resources/js/editor-ui.js'),
                public_path('plugins/LibreLiveTopology/resources/js/editor-versions.js'),
                public_path('plugins/LibreLiveTopology/resources/js/editor-advanced.js'),
            ]));
        @endphp
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/llt-common.js') }}?v={{ $editorAssetVersion }}"></script>
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/ui-helpers.js') }}?v={{ $editorAssetVersion }}"></script>
        <script>
            // Shared polyfill + theme (llt-common.js). Safe if common fails to load.
            if (window.LLT && typeof LLT.ensureUiHelpers === 'function') {
                LLT.ensureUiHelpers();
            } else {
                window.LLTLoading = window.LLTLoading || {};
                ['show', 'hide', 'toggle'].forEach(m => {
                    if (typeof window.LLTLoading[m] !== 'function') {
                        window.LLTLoading[m] = function() {};
                    }
                });
                window.LLTToast = window.LLTToast || {};
                ['success','error','warning','info'].forEach(m => {
                    if (typeof window.LLTToast[m] !== 'function') {
                        window.LLTToast[m] = (msg) => console[m === 'error' ? 'error' : 'log'](msg);
                    }
                });
            }
            if (window.LLT && typeof LLT.observeTheme === 'function') {
                LLT.observeTheme('.editor-container');
            }
        </script>
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/editor-state.js') }}?v={{ $editorAssetVersion }}"></script>
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/topology-ports.js') }}?v={{ $editorAssetVersion }}"></script>
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/topology-layout.js') }}?v={{ $editorAssetVersion }}"></script>
        <script>
            ...
            window.LLT = window.LLT || {};
            window.LLT.EditorState = window.LLT.EditorState || {};
            const __LLTState = window.LLT.EditorState;
            __LLTState.mapId = {{ $map->id ?? 'null' }};
            __LLTState.editorConfig = {
                link_style: '{{ config('librelivetopology.link_style', 'straight') }}',
            };
            __LLTState.uris = {
                map: '{{ url('plugin/LibreLiveTopology/map') }}',
                maps: '{{ url('plugin/LibreLiveTopology/api/maps') }}',
                versions: '{{ url('plugin/LibreLiveTopology/api/versions') }}',
                editor: '{{ url('plugin/LibreLiveTopology/editor') }}',
                devices: '{{ url('plugin/LibreLiveTopology/api/devices') }}',
                device: '{{ url('plugin/LibreLiveTopology/api/device') }}',
                devicePage: '{{ url('device') }}',
                graph: '{{ url('graph') }}',
                replay: '{{ url('plugin/LibreLiveTopology/api/maps') }}/{{ $map->id ?? '' }}/replay',
            };
        </script>
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/editor-canvas.js') }}?v={{ $editorAssetVersion }}"></script>
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/editor-nodes.js') }}?v={{ $editorAssetVersion }}"></script>
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/editor-links.js') }}?v={{ $editorAssetVersion }}"></script>
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/editor-ui.js') }}?v={{ $editorAssetVersion }}"></script>
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/editor-versions.js') }}?v={{ $editorAssetVersion }}"></script>
        <script src="{{ asset('plugins/LibreLiveTopology/resources/js/editor-advanced.js') }}?v={{ $editorAssetVersion }}"></script>
@endsection
