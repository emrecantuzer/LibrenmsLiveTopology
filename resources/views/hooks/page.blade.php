        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title">
                    Network Maps
                    @if($can_create)
                        <a href="{{ url('plugin/LibreLiveTopology') }}" class="btn btn-success btn-sm pull-right">
            ...
            <div class="panel-body">
                        ...
                        @foreach($maps as $map)
                            <div class="col-md-4">
                                <div class="panel panel-default">
                                    <div class="panel-body">
                                        <h4>
                                            <a href="{{ url('plugin/LibreLiveTopology/embed/' . $map->id) }}" aria-label="View map {{ $map->title ?? $map->name }}">
                                                {{ $map->title ?? $map->name }}
                                            </a>
                                        </h4>
                                        <p class="text-muted">
                ...
                @else
                    <div class="alert alert-info">
                        ...
                        @if($can_create)
                            <a href="{{ url('plugin/LibreLiveTopology') }}" class="btn btn-success btn-sm">
...
document.addEventListener('DOMContentLoaded', function() {
    const refresh = () => {
        fetch('{{ url('plugin/LibreLiveTopology/health/stats') }}')
            .then(r => {
