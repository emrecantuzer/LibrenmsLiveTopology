...
.llt-title i { color: #667eea; font-size: 1.75rem; }
.llt-subtitle {
...
.llt-stats-item i { opacity: 0.7; font-size: 1.1rem; }
.llt-stats-item strong {
    color: var(--idx-text);
    font-weight: 600;
    font-size: 1.1rem;
}
...
.llt-filters .form-control {
    background: var(--idx-input-bg);
    border-color: var(--idx-input-border);
    color: var(--idx-text);
}
.llt-filters #map-search { width: 280px; }
.llt-filters #map-filter { width: 180px; }
.llt-filters #map-tag-filter { width: 180px; }
...
@media (max-width: 768px) {
    .llt-header-top { flex-direction: column; align-items: stretch; }
    .llt-header-actions { justify-content: flex-start; }
    .llt-controls { flex-direction: column; align-items: stretch; }
    .llt-stats { justify-content: center; }
    .llt-filters { flex-direction: column; }
    .llt-filters .form-control { width: 100%; }
}
</style>
@endpush

@section('title', 'LibreLiveTopology - Network Maps')
...
<div class="llt-index">
    <div class="container-fluid">
        ...
        <div class="llt-header">
            <div class="llt-header-top">
                <div>
                    <h1 class="llt-title">
                        <i class="fas fa-network-wired" aria-hidden="true"></i>
                        LibreLiveTopology
                    </h1>
                    <p class="llt-subtitle">Real-time network topology visualization with live traffic data</p>
                </div>
                <div class="llt-header-actions">
        ...
        <!-- Controls Bar -->
        <div class="llt-controls">
            <div class="llt-stats">
                <div class="llt-stats-item">
            ...
            @empty
                <div class="col-12">
                    <div class="llt-empty">
                        ...
                        <div class="llt-onboarding-actions">
                            <a href="{{ url('plugin/LibreLiveTopology/templates') }}" class="btn btn-success btn-lg"
                            ...
                            <a href="{{ url('plugin/LibreLiveTopology/diagnostics') }}" class="btn btn-outline-info btn-lg"
        ...
        <!-- Filter Empty State -->
        <div id="map-filter-empty" class="llt-empty" style="display: none;">
            <div class="llt-empty-icon">
        ...
        <div class="modal-content">
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane" id="customPane" role="tabpanel">
                    <form method="POST" action="{{ url('plugin/LibreLiveTopology/map') }}" id="createMapForm" novalidate>
                        @csrf
                        <div class="modal-body">
...
<div class="modal fade" id="importMapModal" tabindex="-1" role="dialog" aria-labelledby="importMapModalTitle">
    <div class="modal-dialog modal-dialog-centered">
        <form method="POST" action="{{ url('plugin/LibreLiveTopology/api/import') }}" class="modal-content" id="importMapForm" enctype="multipart/form-data">
            @csrf
            <div class="modal-header">
...
@section('scripts')
<script src="{{ asset('plugins/LibreLiveTopology/resources/js/llt-common.js') }}"></script>
<script src="{{ asset('plugins/LibreLiveTopology/resources/js/ui-helpers.js') }}"></script>
<script>
    if (window.LLT && typeof LLT.ensureUiHelpers === 'function') {
        ...
        ['success','error','warning','info'].forEach(m => {
            if (typeof window.LLTToast[m] !== 'function') {
                window.LLTToast[m] = _c(m === 'error' ? 'error' : 'log');
            }
        });
    }
    if (window.LLT && typeof LLT.observeTheme === 'function') {
        LLT.observeTheme('.llt-index');
    }
</script>
<script>
...
function getCsrfToken() {
    if (window.LLT && typeof LLT.getCsrfToken === 'function') {
        return LLT.getCsrfToken();
    }
...
$('#createMapForm').on('submit', function(e) {
    ...
    fetch('{{ url("plugin/LibreLiveTopology/map") }}', {
        method: 'POST',
        body: formData,
        headers: {
    ...
    .then(data => {
        if (data.success) {
            LLTToast.success('Map created successfully!');
            ...
            LLTToast.error('Error: ' + (data.message || 'Unknown error'));
        }
    })
    .catch(error => {
        LLTToast.error('Error: ' + error.message);
    })
    .finally(() => {
...
$('#importMapForm').on('submit', function(e) {
    ...
    fetch('{{ url("plugin/LibreLiveTopology/api/import") }}', {
        method: 'POST',
        body: formData,
        headers: {
            'X-CSRF-TOKEN': getCsrfToken()
    ...
    .then(data => {
        if (data.success) {
            LLTToast.success('Map imported successfully!');
            $('#importMapModal').modal('hide');
            location.reload();
        } else {
            LLTToast.error('Error: ' + (data.message || data.error || 'Unknown error'));
        }
    })
    .catch(error => {
        LLTToast.error('Error: ' + error.message);
...
document.getElementById('confirmDeleteMapBtn')?.addEventListener('click', function() {
    ...
    form.action = '{{ url("plugin/LibreLiveTopology/map") }}/' + pendingDeleteMapId;
    form.submit();
});
...
function loadTemplates() {
    ...
    fetch('{{ url("plugin/LibreLiveTopology/templates") }}', {
        headers: {
...
function createMapFromTemplate(templateId, mapName) {
    ...
    fetch(`{{ url("plugin/LibreLiveTopology/templates") }}/${templateId}/create-map`, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        ...
        if (mapId) {
            window.location.href = '{{ url("plugin/LibreLiveTopology/editor") }}/' + mapId;
            return;
        }
        // No map ID in response — show toast and reload to see the new map.
        try { LLTToast.success('Map created from template!'); } catch(e) { console.log('Map created from template!'); }
        try { $('#createMapModal').modal('hide'); } catch(e) {}
        location.reload();
    })
    .catch(err => {
        LLTToast.error('Error: ' + err.message);
