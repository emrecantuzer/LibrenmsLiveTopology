    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div>
                    <a href="{{ url('plugin/LibreLiveTopology') }}" class="btn btn-secondary">
                        <i class="fas fa-arrow-left" aria-hidden="true"></i> Back to Maps
                    </a>
                    <a href="{{ url('plugin/LibreLiveTopology/embed/' . $map->id) }}" class="btn btn-default" target="_blank" rel="noopener noreferrer" aria-label="Open live map {{ $map->title ?? $map->name }}">
                ...
                <div class="card-body text-center">
                    <div style="width: {{ $map->width }}px; height: {{ $map->height }}px; border: 2px solid #dee2e6; margin: 0 auto; display: flex; align-items: center; justify-content: center; background-color: #f8f9fa;">
                        <div class="text-muted">
                            ...
                            <a href="{{ url('plugin/LibreLiveTopology/embed/' . $map->id) }}" class="btn btn-default btn-sm" target="_blank" rel="noopener noreferrer" aria-label="Open live map preview for {{ $map->title ?? $map->name }}">
                                <i class="fas fa-external-link-alt" aria-hidden="true"></i> Open Live Map
                            </a>
                        </div>
                    </div>
