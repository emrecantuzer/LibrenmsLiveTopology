...
use LibreNMS\Plugins\LibreLiveTopology\Models\MapTemplate;
...
class MapTemplateController extends Controller
{
    ...
    public function createFromTemplate(Request $request, int $id): JsonResponse
    {
        ...
        try {
            ...
            Log::error('Failed to create map from template', ['error' => $e->getMessage()]);
            return response()->json([
                'success' => false,
                'message' => 'Failed to create map from template',
            ], 500);
        }
