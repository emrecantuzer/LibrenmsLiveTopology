...
class Node extends Model
{
    ...
    public static function preloadDevices(array $deviceIds): void
    {
        ...
        if (class_exists('App\\Models\\Device')) {
            $devices = \App\Models\Device::whereIn('device_id', $remaining)
                ->get(['device_id', 'hostname', 'sysName', 'status'])
                ->keyBy('device_id');
    ...
    public function getDeviceSysnameAttribute()
    {
        return $this->resolveDevice($this->device_id)['sysName'] ?? null;
    }
    ...
    private function fetchDevice(int $deviceId): ?array
    {
        ...
        $row = dbFetchRow("SELECT hostname, sysName, status FROM devices WHERE device_id = ?", [$deviceId]);
        return $row ?: null;
    }
    ...
    public function toArray()
    {
        ...
        $data['device_sysname'] = $this->device_sysname;
