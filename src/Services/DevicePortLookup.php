...
class DevicePortLookup
{
    ...
    {
        $cacheKey = "librelivetopology.device_ports.{$deviceId}";
            ...
            try {
                if (class_exists('\App\Models\Port')) {
                    return \App\Models\Port::where('device_id', $deviceId)
                        ->where('deleted', 0)
                        ->select('port_id', 'ifName', 'ifIndex', 'ifOperStatus', 'ifAdminStatus', 'ifSpeed')
                        ->orderBy('ifName')
                        ->get()
                        ->toArray();
                }
    ...
    public function deviceAutocomplete(string $query, int $limit = 20): array
    {
        ...
        $cacheKey = "librelivetopology.device_search." . hash('sha256', $query);
        $cacheTtl = config('librelivetopology.cache_ttl', 300);
    ...
    {
        $cacheKey = "librelivetopology.all_devices";
        $cacheTtl = config('librelivetopology.cache_ttl', 300);
