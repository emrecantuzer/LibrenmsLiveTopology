...
class NodeDataService
{
    ...
    {
        $metricsMap = config('librelivetopology.show_node_metrics', true)
    ...
    public function buildLinkData(Map $map): array
    {
        ...
        foreach ($map->links as $link) {
            ...
            if ($demoMode || (!$link->port_id_a && !$link->port_id_b)) {
                ...
                $data = $this->portUtil->linkUtilBits([
                    'port_id_a' => $link->port_id_a,
                    'port_id_b' => $link->port_id_b,
                    'bandwidth_bps' => $link->bandwidth_bps,
                ]);
                $utilization = (float) ($data['pct'] ?? 0);
                $data['latency_ms'] = null;
                $data['jitter_ms'] = null;
                $data['packet_loss'] = null;
                $data['health_score'] = (int) max(0, min(100, 100 - max(0, $utilization - 70) * 3.33));
                $linkData[$link->id] = $data;
            }
        }
        return $linkData;
    }
    ...
    private function generateDemoLinkData($link): array
    {
        ...
        return [
            ...
            'jitter_ms' => null,
            'packet_loss' => null,
            'health_score' => (int) max(0, min(100, 100 - max(0, $utilization - 70) * 3.33)),
            'source' => 'demo',
        ];
    }
