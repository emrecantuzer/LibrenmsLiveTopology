...
class DiagnosticsIntegrationTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->rrdDir = sys_get_temp_dir() . '/llt_integrity_' . bin2hex(random_bytes(4));
    ...
    private function createSchema(): void
    {
        ...
        $schema->create('ports', function (Blueprint $t) {
            ...
            $t->unsignedBigInteger('ifSpeed')->nullable();
        });

        // llt tables (no FKs so orphan rows can be inserted).
    ...
    {
        $id = \Illuminate\Support\Facades\DB::table('llt_maps')->insertGetId([
            'name' => $name,
            'title' => $title,
    ...
    {
        return \Illuminate\Support\Facades\DB::table('llt_nodes')->insertGetId([
            'map_id' => $mapId,
            'label' => 'N' . $deviceId,
            'x' => 0,
            'y' => 0,
    ...
    {
        return \Illuminate\Support\Facades\DB::table('llt_links')->insertGetId([
            'map_id' => $mapId,
            'src_node_id' => $srcNode,
            'dst_node_id' => $dstNode,
            'port_id_a' => $portA,
    ...
    public function test_detects_broken_port_missing_rrd_and_orphans(): void
    {
        ...
        \Illuminate\Support\Facades\DB::table('llt_links')->insert([
            'map_id' => 999999,
            'src_node_id' => 1,
            'dst_node_id' => 2,
            'created_at' => date('Y-m-d H:i:s'),
