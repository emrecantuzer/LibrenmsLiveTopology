...
namespace Illuminate\Console {
    if (!class_exists('Illuminate\Console\Command')) {
        class Command extends \Symfony\Component\Console\Command\Command
        {
            ...
            {
                $this->lltInput = $input;
                $this->lltOutput = $output;
            }

            public function argument(string $key)
            {
                return $this->lltInput->getArgument($key);
            }

            public function option(string $key)
            {
                return $this->lltInput->getOption($key);
            }

            public function info(string $message): void
            {
                $this->lltOutput->writeln("<info>{$message}</info>");
            }

            public function error(string $message): void
            {
                $this->lltOutput->writeln("<error>{$message}</error>");
            }

            public function warn(string $message): void
            {
                $this->lltOutput->writeln("<comment>{$message}</comment>");
            }
        }
    }
}
