...
class DocsPathsTest extends TestCase
{
    ...
    public function testDocsUseV2Paths()
    {
        foreach ($this->docs as $file) {
            ...
            $this->assertStringContainsString('plugin/LibreLiveTopology', $content, "$file should reference v2 path");
            $this->assertStringNotContainsString('plugins/librelivetopology', $content, "$file should not reference legacy path");
        }
    }
    ...
    public function testTemplatesPreferV2Paths()
    {
        ...
        foreach ($files as $file) {
            $this->assertFileExists($file, "$file should exist");
            $content = file_get_contents($file);
            $this->assertStringNotContainsString('/plugins/librelivetopology', $content, "$file contains legacy path");
        }
    }
}
