...
class E2EInstallationWorkflowTest extends TestCase
{
    ...
    public function test_quick_install_script_guards_native_root_runs(): void
    {
        ...
        $this->assertStringContainsString('LIBRELIVETOPOLOGY_ALLOW_ROOT', $scriptContent);
    }
    ...
    public function test_quick_install_normalizes_duplicate_plugin_rows(): void
    {
        ...
        $this->assertStringContainsString('Normalizing LibreLiveTopology plugin registration', $scriptContent);
        $this->assertStringContainsString("where('plugin_name', 'LibreLiveTopology')", $scriptContent);
        ...
        $this->assertStringContainsString('Cleaned $deleted duplicate LibreLiveTopology plugin row(s)', $scriptContent);
    }
    ...
    public function test_install_docs_cover_plugin_duplicates_and_json_collation_warnings(): void
    {
        ...
        foreach ([$install, $readme, $deployment] as $content) {
            ...
            $this->assertStringContainsString('llt_map_templates.config', $content);
            $this->assertStringContainsString('llt_nodes.meta', $content);
            $this->assertStringContainsString('llt_maps.options', $content);
            $this->assertStringContainsString('llt_links.style', $content);
        }
    }
    ...
    public function test_database_setup_creates_tables(): void
    {
        ...
        $this->assertStringContainsString('llt_maps', $scriptContent);
        $this->assertStringContainsString('llt_nodes', $scriptContent);
        $this->assertStringContainsString('llt_links', $scriptContent);
    }
    ...
    public function test_install_routes_defined(): void
    {
        ...
        $this->assertStringContainsString("Route::get('plugin/LibreLiveTopology/install'", $content);
        $this->assertStringContainsString("Route::post('plugin/LibreLiveTopology/install'", $content);
    }
    ...
    public function test_installation_detection_in_page_controller(): void
    {
        ...
        $this->assertStringContainsString("route('librelivetopology.install')", $content);
    }
    ...
    public function test_installation_flow_complete(): void
    {
        ...
        $hasRoutes = strpos(file_get_contents(__DIR__ . '/../routes/web.php'), 'librelivetopology.install') !== false;
    ...
    public function test_fallback_mechanisms_present(): void
    {
        ...
        $hasInstallRoutes = strpos($content, 'librelivetopology.install') !== false;
