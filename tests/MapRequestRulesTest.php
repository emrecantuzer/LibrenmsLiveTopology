...
class MapRequestRulesTest extends TestCase
{
    ...
    public function test_all_link_write_endpoints_preserve_curved_style_and_via_points(): void
    {
        $content = file_get_contents(__DIR__ . '/../src/Http/Controllers/MapLinkController.php');

        $this->assertStringContainsString("'links.*.style' => 'nullable|array:via_style,via_points,color,width'", $content);
        $this->assertStringContainsString("'links.*.style.via_style' => 'nullable|string|in:straight,angled,curved'", $content);
        $this->assertStringContainsString("'style' => 'nullable|array:via_style,via_points,color,width'", $content);
        $this->assertStringContainsString("'style' => 'sometimes|array:via_style,via_points,color,width'", $content);
        $this->assertGreaterThanOrEqual(3, substr_count($content, "style.via_points.*.x' => 'required|numeric|min:0|max:10000'"));
    }
