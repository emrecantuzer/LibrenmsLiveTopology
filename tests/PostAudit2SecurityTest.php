...
class PostAudit2SecurityTest extends TestCase
{
    ...
    public function test_create_links_logs_dropped_links(): void
    {
        $content = file_get_contents($this->base . '/src/Services/MapService.php');
        $this->assertStringContainsString(
            "Log::warning('LibreLiveTopology: dropped link",
            $content,
    ...
    // ── Controller error responses: no raw exception disclosure ─────────

    public function test_map_save_genericizes_unexpected_errors(): void
    {
        $content = file_get_contents($this->base . '/src/Http/Controllers/MapController.php');
        $this->assertStringContainsString("'message' => 'Failed to save map.'", $content);
        $this->assertStringNotContainsString("'message' => 'Failed to save map: ' . \$e->getMessage()", $content);
    }

    public function test_auto_discovery_genericizes_unexpected_errors(): void
    {
        $content = file_get_contents($this->base . '/src/Http/Controllers/MapController.php');
        $this->assertStringContainsString("'message' => 'Auto-discovery failed.'", $content);
        $this->assertStringNotContainsString("'message' => 'Auto-discovery failed: ' . \$e->getMessage()", $content);
    }

    public function test_install_genericizes_unexpected_errors(): void
    {
        $content = file_get_contents($this->base . '/src/Http/Controllers/InstallController.php');
        $this->assertStringContainsString("'message' => 'Installation failed.'", $content);
        $this->assertStringNotContainsString("'message' => 'Installation failed: ' . \$e->getMessage()", $content);
    }

    public function test_map_import_upload_limit_stays_within_sonar_threshold(): void
    {
        $content = file_get_contents($this->base . '/src/Http/Controllers/RenderController.php');
        $this->assertStringContainsString("'file' => 'required|file|mimes:json|max:8192'", $content);
        $this->assertStringNotContainsString('max:10240', $content);
    }
    ...
    public function test_fetch_device_restricts_eloquent_columns(): void
    {
        $content = file_get_contents($this->base . '/src/Models/Node.php');
        $this->assertStringContainsString(
            "->first(['device_id', 'hostname', 'sysName', 'status'])",
            $content,
