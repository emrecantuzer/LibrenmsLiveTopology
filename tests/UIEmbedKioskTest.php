...
class UIEmbedKioskTest extends TestCase
{
    ...
    public function test_embed_includes_kiosk_cycle_logic(): void
    {
        ...
        $this->assertStringContainsString('if (!LLT_CONFIG.kioskEnabled) return;', $this->content);
    ...
    {
        $this->assertStringContainsString("window.open(url, LLT_CONFIG.linkTarget || '_blank');", $this->content);
        $this->assertStringContainsString("window.open(url, LLT_CONFIG.linkTarget || '_blank');", $this->content);
    }
    ...
    public function test_embed_escape_keyhandler_does_not_show_chrome_before_toggle(): void
    {
        ...
        $start = strpos($this->content, 'function initKioskMode()');
        $end = strpos($this->content, 'function initNavControls()', $start);
        $kiosk = substr($this->content, $start, $end - $start);
        $this->assertSame(
            1,
            substr_count($kiosk, "addEventListener('keydown'"),
