...
test_service_health() {
    ...
    if $COMPOSE_CMD -p "$TEST_PREFIX" -f docker-compose.simple.yml exec -T librenms \
        test -d /opt/librenms/html/plugins/LibreLiveTopology; then
...
test_plugin_installation() {
    ...
    if $COMPOSE_CMD -p "$TEST_PREFIX" -f docker-compose.simple.yml exec -T librenms \
        test -f /opt/librenms/html/plugins/LibreLiveTopology/vendor/autoload.php; then
        log_pass "Composer dependencies installed"
    else
        log_info "Installing composer dependencies..."
        if $COMPOSE_CMD -p "$TEST_PREFIX" -f docker-compose.simple.yml exec -T librenms \
            bash -c "cd /opt/librenms/html/plugins/LibreLiveTopology && composer install --no-dev" &>/dev/null; then
    ...
    for file in "${plugin_files[@]}"; do
        if $COMPOSE_CMD -p "$TEST_PREFIX" -f docker-compose.simple.yml exec -T librenms \
            test -f "/opt/librenms/html/plugins/LibreLiveTopology/$file"; then
...
main() {
    echo "========================================"
    echo "LibreLiveTopology Docker Test Suite"
    echo "========================================"
    echo
