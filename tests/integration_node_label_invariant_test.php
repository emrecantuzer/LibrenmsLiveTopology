 ...
 *   docker exec -u librenms librenms-dev php \
     /opt/librenms/html/plugins/LibreLiveTopology/tests/integration_node_label_invariant_test.php
 *
...
use LibreNMS\Plugins\LibreLiveTopology\Services\NodeLabelNormalizer;
...
try {
    ...
    check(!class_exists(\LibreNMS\Plugins\LibreLiveTopology\Http\Requests\CreateLinkRequest::class),
        'CreateLinkRequest class removed (PR #27)');
    check(!class_exists(\LibreNMS\Plugins\LibreLiveTopology\Http\Requests\CreateNodeRequest::class),
        'CreateNodeRequest class removed (PR #27)');
    check(!class_exists(\LibreNMS\Plugins\LibreLiveTopology\Services\MapCacheService::class),
    ...
    // ----- Create a test map through the kernel (POST /plugin/LibreLiveTopology/map) -----
    [$status, $json] = dispatch('POST', '/plugin/LibreLiveTopology/map', [
        'name' => $mapName,
    ...
    [$status, $json] = dispatch('POST', "/plugin/LibreLiveTopology/map/$mapId/node", [
        'label' => '<b></b>', 'x' => 10, 'y' => 20,
    ]);
    ...
    [$status, $json] = dispatch('PATCH', "/plugin/LibreLiveTopology/map/$mapId/node/$nodeAId", [
        'label' => '<i></i>',
    ]);
    ...
    [$status, $json] = dispatch('POST', "/plugin/LibreLiveTopology/api/maps/$mapId/save", [
        'nodes' => [
    ...
    check(!method_exists(\LibreNMS\Plugins\LibreLiveTopology\Http\Controllers\MapVersionController::class, 'autoSave'),
        'MapVersionController::autoSave method removed (PR #26)');
    check(is_null(config('librelivetopology.auto_save')) && is_null(config('librelivetopology.auto_save_interval')),
        'auto_save config keys removed (PR #26)');

    // ----- PR #25: debug-gated logging config key + embed view formatting -----
    check(array_key_exists('debug', config('librelivetopology') ?? []), 'librelivetopology.debug config key exists (PR #25)');
    $embedView = file_get_contents('/opt/librenms/html/plugins/LibreLiveTopology/resources/views/embed.blade.php');
