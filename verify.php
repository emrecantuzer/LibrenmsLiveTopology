...
<?php
/**
 * LibreLiveTopology Installation Verification & Repair Script
...
if (isset($options['help'])) {
    echo "LibreLiveTopology Verification Script\n";
...
// Start verification
if (!$QUIET_MODE) {
    echo "🔍 LibreLiveTopology Installation Verification\n";
    echo "==========================================\n";
    if ($FIX_MODE) {
...
$hook_based_files = [
    'src/LibreLiveTopologyProvider.php',
    'src/Hooks/MenuEntry.php',
    'src/Hooks/Settings.php'
];

$traditional_files = [
    'src/LibreLiveTopology.php',
    'composer.json',
    'routes/web.php',
...
check_and_fix('Composer dependencies', function() use ($plugin_path) {
    ...
    if (!class_exists(\Symfony\Component\Process\Process::class)) {
        chdir($original_dir);
        return "Symfony Process is unavailable; install dependencies manually";
    }
    ...
    if ($process->isSuccessful() && file_exists($plugin_path . '/vendor/autoload.php')) {
        return true;
    }

    return "Failed to install dependencies: " . trim($process->getErrorOutput() ?: $process->getOutput());
});
...
check_and_fix('Log directory', function() {
    ...
    $test_file = '/tmp/librelivetopology.log';
    if (touch($test_file)) {
        unlink($test_file);
        return true;
    }
...
check_and_fix('LibreNMS Integration', function() use ($plugin_path, $architecture) {
    if ($architecture === 'hook-based') {
        ...
        $hooks_dir = $plugin_path . '/app/Plugins/LibreLiveTopology';
        if (!is_dir($hooks_dir)) {
        ...
        if (file_exists($plugin_path . '/src/LibreLiveTopology.php')) {
            return true;
        }
...
if ($issues_found === 0) {
    ...
    echo "\n🎉 LibreLiveTopology is ready to use!\n";
    echo "Visit: https://your-librenms/plugin/LibreLiveTopology\n";
    exit(0);
} else {
    ...
    echo "\n";
    echo "For help, see INSTALL.md and docs/EMBED.md in this repository.\n";
